...
> **Canonical reference: [`docs/ui-patterns/page-header.md`](./page-header.md).** That doc has the full anatomy, variants, surface tokens, common mistakes, and a copy-paste object-home/list-view variant. Read it before building the header. The summary below is list-view-specific context.
...
SLDS: Use the object-home / list-view variant in [`docs/ui-patterns/page-header.md`](./page-header.md). The title column **must** use `slds-has-flexi-truncate` (not `slds-page-header__col-title`) and the actions column **must** use `slds-no-flex slds-grid slds-align-top` (not `slds-page-header__col-actions`) so the title truncates instead of pushing the action buttons off-screen.
