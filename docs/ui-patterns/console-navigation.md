...
> **Canonical reference: [`docs/ui-patterns/page-header.md`](./page-header.md).** That doc is the single source of truth for page header markup, variants, surface tokens, common mistakes, and a copy-paste canonical example. The section below is the console-record context for the same pattern; read both.
