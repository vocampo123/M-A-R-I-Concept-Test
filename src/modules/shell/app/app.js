...
import MembershipCenter from 'page/membershipCenter';
import MembershipSetup from 'page/membershipSetup';
import MembersList from 'page/membersList';
import MemberDetail from 'page/memberDetail';
import LoyaltyMemberDetail from 'page/loyaltyMemberDetail';
import NotFound from 'page/notFound';

/** Option A: explicit registration – add one import + one entry here when adding a route */
const ROUTE_COMPONENTS = {
    'page-membership-center': MembershipCenter,
    'page-membership-setup': MembershipSetup,
    'page-members-list': MembersList,
    'page-member-detail': MemberDetail,
    'page-loyalty-member-detail': LoyaltyMemberDetail,
};
...
export default class App extends LightningElement {
    ...
    get consoleSubtabs() {
        if (this.route?.component !== 'page-member-detail') {
            return [];
        }
        ...
        return [
            {
                id: `member-${memberId}`,
                parentPage: 'members',
                label: member?.name ?? 'Member Record',
                href: linkHref(path),
                path,
    ...
    get currentSubtabId() {
        return this.consoleSubtabs[0]?.id ?? null;
    }
    ...
    handleNavNavigate(event) {
        ...
        const pagePath = page ? NAV_PAGE_TO_PATH[page] : '/';
        navigate(pagePath);
    }
