...
export default class GlobalNavigation extends LightningElement {
    ...
    get navItemsWithActive() {
        return (this.navItems || []).map((item) => {
            ...
            const hasSubtabs = isActive && this.hasSubtabs;
            return {
                ...item,
                isActive,
                tabClass: `${base}${isActive ? ' slds-is-active' : ''}${hasSubtabs ? ' slds-has-sub-tabs' : ''}`,
    ...
    get subtabsWithActive() {
        return (this.subtabs || []).map((item) => {
            const isActive = item.id === this.currentSubtabId;
            return {
                ...item,
                isActive,
                tabClass: `slds-context-bar__item slds-context-bar__item_tab${isActive ? ' slds-is-active' : ''}`,
                ariaCurrent: isActive ? 'page' : null,
            };
        });
    }
